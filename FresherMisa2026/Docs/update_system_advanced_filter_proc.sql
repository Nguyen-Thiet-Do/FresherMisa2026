...
CREATE PROCEDURE `Proc_pa_salary_composition_system_AdvancedFilterPaging`(
  ...
  IN v_component_type_id VARCHAR(36),    -- Phần 2: loại thành phần (NULL = không lọc)
  IN v_filters           JSON,           -- Phần 3: field conditions
  IN v_filter_logic      TINYINT         -- 0 = AND (default), 1 = OR cho Phần 3
)
BEGIN
  ...
  DECLARE v_where      TEXT         DEFAULT 'sc.`SystemCompositionID` NOT IN (SELECT SystemCompositionID FROM pa_salary_composition WHERE Source = 2 AND IsDeleted = FALSE)';
  ...
  IF v_component_type_id IS NOT NULL AND v_component_type_id != '' THEN
    SET v_where = CONCAT(v_where, ' AND sc.`ComponentTypeID` = ', QUOTE(v_component_type_id));
  END IF;

  -- Phần 3: lọc nâng cao các trường
  IF v_filters IS NOT NULL AND JSON_LENGTH(v_filters) > 0 THEN
    ...
    IF v_user_conds IS NOT NULL THEN
      ...
      SET v_where = CONCAT(v_where, '(', v_user_conds, ')');
    END IF;
  END IF;

  SET v_where = CONCAT('WHERE ', v_where);
  ...
  IF v_sort IS NOT NULL AND v_sort != '' THEN
    ...
    END WHILE;
  ELSE
    SET v_order_by = 'ORDER BY sc.`CreateDate` DESC';
  END IF;
  ...
  SET @v_sql = CONCAT(
    ...
    v_where, ' ', v_order_by, ' LIMIT ', v_pageSize, ' OFFSET ', v_offset
  );
  SET @v_count_sql = CONCAT('SELECT COUNT(*) FROM `pa_salary_composition_system` sc ', v_where);
